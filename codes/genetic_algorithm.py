...
class VRP:
    ...
    """
    def __init__(self, customers, distances, time_matrix, vehicle):
...
class Modele_genetic():
    ...
    def distance(self,VRP,position1,position2): # Calculate the distance between two positions
    ...
    def fitness(self,vrp,chromosome):# Calculate the fitness of a chromosome, here the fitness is determined by the reciprocal of cost

        cost1=cost(vrp,chromosome)
        fitness=1/cost1
        return fitness
...
class VRP_GA():
    ...
    def evolution(self,modele_genetic,num_parent,population,rate_mutation,num_pop): # Realize a generation, including the mating, the mutation, the elimination and the regeneration

        def binary_tournement(self,modele_genetic,population,num_parent):# Select certain individuals as parents by their fitness
            parents=[]            
        ...
        def pop_crossover(self,modele_genetic,parents,population):# Realize mating between parents 
            for i in len(parents): 
                parent=random.sample(parents,2)
                population.append(modele_genetic.crossover(parent[0],parent[1]))
            return population
        
        def pop_mutation(self,modele_genetic,population,rate_mutation):# Realize mutation for all members in the population
            for i in population:
                i=modele_genetic.mutation(i,rate_mutation)
            return population
        
        def eliminate(self,modele_genetic,population): # Eliminate the less strong half of the population
            list_fitness=[]
